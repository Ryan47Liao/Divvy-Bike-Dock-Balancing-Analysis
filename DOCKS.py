...
class DOCKs:
    ...
    def Data_Transform(self,trip_subset,date):
        ...
        trip_subset = trip_subset.merge(self.df_Station_final[['station_id','Nbh_id','com_id']],left_on='start_station_id',right_on='station_id')
        trip_subset = trip_subset.rename(columns={'Nbh_id':'start_Nbh_id','com_id':'start_com_id'}).drop(columns = ['station_id'],axis = 1)
        trip_subset = trip_subset.merge(self.df_Station_final[['station_id','Nbh_id','com_id']],left_on='end_station_id',right_on='station_id')
    ...
    def _docks_update(self,time_start=None,time_mins = 1):
        ...
        current_docks = {k:list(v.values())[0] for k,v in self.DF_Dock.tail(1).to_dict().items()}
    ...
    def Capacity_Analysis(self):
        ...
        df_Capacity['daily_capacity_high'] = pd.to_numeric(list(df_Capacity['daily_capacity_high']))
        df_Capacity['daily_capacity_mean'] = list(Dock_Normal.mean())
        df_Capacity['daily_capacity_median'] = list(Dock_Normal.median())
        df_Station_cluster_capacity = self.df_Station_final.merge(df_Capacity,left_on='Nbh_id',right_on='Nbh_id')
        self.df_Station_cluster_capacity = df_Station_cluster_capacity
        return df_Station_cluster_capacity
    
    def Capacity_mean_dataviz(self,bins=[-1,-0.1,-0.01,0.01,0.1,0.2,0.5],df_Station_cluster_capacity=None,
                                show_detail = True,figsize=(24,10),palette = 'coolwarm'):
    ...
    def Capacity_high_dataviz(self,bins=[-0.01,0,0.1,0.2,0.3,0.4,1],df_Station_cluster_capacity=None,
                              show_detail = True,figsize=(24,10),palette = 'magma'):
        if df_Station_cluster_capacity is None:
            if self.df_Station_cluster_capacity is None:
                df_Station_cluster_capacity = self.Capacity_Analysis()
            else:
